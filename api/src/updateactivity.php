...
use FirebaseJWT\Key;
/** 
 * update an activity, a valid jwt token is required
 * @author Noorullah Niamatullah
 */
class UpdateActivity extends Endpoint
{   private $userid=32;
    ...
    private function validateUpdateParams(){
      ...
      if(!isset($_POST['activityCategory'],$_POST["activityName"],$_POST["date"],$_POST["time"],$_POST["duration"],$_POST["activityNotes"])){
        throw new ClientException("please supply required parameters", 400);
      }
    }

    protected function initialiseSQL() {   
      $sql = "UPDATE activity  set userID =:userID, activityCategory =:activityCategory ,activityName =:activityName,date =:date, time=:time, duration =:duration ,activityNotes =:activityNotes
              WHERE activityID =:activityID";
      $this->setSQL($sql);
      $this->setSQLParams(['userID'=>$this->userid ,'activityCategory'=> $_POST['activityCategory'],'activityName'=> $_POST['activityName'],'date'=> $_POST['date'],'time'=> $_POST['time'],'duration'=> $_POST['duration'],'activityNotes'=> $_POST['activityNotes'],'activityID'=> $_POST['activityID']]);
    }
