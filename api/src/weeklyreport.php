...
use FirebaseJWT\Key;
/** 
 * get the durtion of each activity for a given week
 * @author Noorullah Niamatullah
 */
class Weeklyreport extends Endpoint
{   private $userid=32;
    ...
    private function validateUpdateParams(){
      ...
      if(!isset($_POST["startingweek"],$_POST["endingweek"])){
        throw new ClientException("please supply required parameters", 400);
      }
    }

    protected function initialiseSQL() {   
      $sql = "SELECT activityCategory , duration
              FROM activity where userID=:userID and Date(date) between :startingweek and :endingweek";
      $this->setSQL($sql);
      $this->setSQLParams(['userID'=>$this->userid,'startingweek'=>$_POST['startingweek'],'endingweek'=>$_POST['endingweek']]);
    }
